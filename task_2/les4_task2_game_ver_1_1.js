...
const player = {
    ...
    chekMove(direction) {
        if ((this.x === 0 && direction === 4) ||
            (this.y === 0 && direction === 8) ||
            (1 + this.x >= this.settings.rowCount && direction === 6) ||
            (1 + this.y >= this.settings.colCount && direction === 2)) {

            alert('Нельзя выходить за пределы игрового поля');
            this.chekMovePass = false;
        } else this.chekMovePass = true;
    },
};
...
const game = {
    ...
    run() {
        ...
        while (true) {
            ...
            this.player.chekMove(direction);

            if (this.player.chekMovePass === true) this.player.move(direction);
        }
    },
